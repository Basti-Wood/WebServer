...
public:
	...
	void buildResponse(HTTPResponse& response, bool headers_only);
private:
	...
	// appends body bytes, spilling to a temp file past a threshold
	void _appendBody(const std::string& chunk);
	void _closeSpool();
    ...
    // large bodies are spilled to a temp file instead of RAM
    std::string _spool_path;
    int         _spool_fd;
};
